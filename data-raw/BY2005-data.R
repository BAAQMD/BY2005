...
BY2005_P_data <-
  import_annual_data_("BY2005_Pointtpd.csv") %>%
  mutate(cat_id = str_c("P", cat_id))

BY2005_A_data <-
  import_annual_data_("BY2005_Areatpd.csv") %>%
  mutate(cat_id = str_c("A", cat_id))

BY2005_annual <-
  bind_rows(BY2005_P_data, BY2005_A_data) %>%
  convert_units(from = "tons/day", to = "tons/yr") %>%
  select(year, cat_id, pol_abbr, ems_qty, ems_unit) %>%
  with_comment("BY2005 area source emissions, by category, 1990—2030.")
